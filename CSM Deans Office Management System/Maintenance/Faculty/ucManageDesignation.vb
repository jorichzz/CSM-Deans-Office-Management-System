Public Class ucManageDesignation
    ...
    Sub loadDocTypeID()
        Try
            dbConnect()
            Dim query As String = "SELECT Id from designation ORDER BY Id DESC LIMIT 1"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
                    txtDocTypeID.Text = "DSGNTN" & Val(Mid(reader.GetString(0), 7, 11)) + 1
                End While
            Else
                txtDocTypeID.Text = "DSGNTN1001"
            End If
            dbDisconnect()
    ...
    Sub loadDataDisplay()
        ...
        Try
            Dim i As Integer = 0
            dbConnect()
            Dim query As String = "SELECT Id, name FROM designation WHERE Status = '" & status & "'"
            ExecuteQuery(query)
    ...
    Sub transferName()
        Try
            dbConnect()
            Dim query As String = "SELECT * FROM designation WHERE Id = '" & Id & "'"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
    ...
    Sub delete()
        If MsgBox("Are you sure you want to delete?", vbYesNo, "Dean's Record Tracking System") = vbYes Then
            Try
                Dim status = "DELETED"
                dbConnect()
                Dim query As String = "UPDATE designation SET Status = '" & status & "' WHERE Id = '" & Id & "'"
                ExecuteQuery(query)
                MsgBox("Designation successfully Removed!", vbInformation, "Deleted")
                loadDataDisplay()
                clear()
                lockEditDel()
                dbDisconnect()
    ...
    Sub save()
        ...
        Else
            If edit = True Then
                ...
                Try
                    dbConnect()
                    Dim query As String = "UPDATE designation SET name = '" & name & "' WHERE Id = '" & id & "' "
                    ExecuteQuery(query)
                    MsgBox("Designation successfully Updated!", vbInformation, "Updated")
                    dbDisconnect()
                Catch ex As Exception
                    clear()
                    lockEditDel()
            ...
            Else
                Try
                    dbConnect()
                    Dim query As String = "INSERT INTO designation SET Id  = '" & id & "', name ='" & name & "', Status ='ACTIVE', date_created ='" & dcreated & "'"

                    ExecuteQuery(query)
                    MsgBox("Designation successfully Registered!", vbInformation, "Saved")
