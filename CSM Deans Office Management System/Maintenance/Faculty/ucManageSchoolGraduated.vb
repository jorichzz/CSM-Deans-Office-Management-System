Public Class ucManageSchoolGraduated
    Dim edit As Boolean = False
    Dim Id As String
    Private Sub btnBack_Click(sender As Object, e As EventArgs) Handles btnBack.Click
        If schoolgrad = True Then
    ...
    Sub loadDocTypeID()
        Try
            dbConnect()
            Dim query As String = "SELECT Id from schoolgrad ORDER BY Id DESC LIMIT 1"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
                    txtDocTypeID.Text = "SCHL" & Val(Mid(reader.GetString(0), 5, 9)) + 1
                End While
            Else
                txtDocTypeID.Text = "SCHL1001"
            End If
            dbDisconnect()
    ...
    Sub loadDataDisplay()
        ...
        Try
            Dim i As Integer = 0
            dbConnect()
            Dim query As String = "SELECT Id, name FROM schoolgrad WHERE Status = '" & status & "'"
            ExecuteQuery(query)
    ...
    Sub transferName()
        Try
            dbConnect()
            Dim query As String = "SELECT * FROM schoolgrad WHERE Id = '" & Id & "'"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
    ...
    Sub delete()
        If MsgBox("Are you sure you want to delete?", vbYesNo, "Dean's Record Tracking System") = vbYes Then
            Try
                Dim status = "DELETED"
                dbConnect()
                Dim query As String = "UPDATE schoolgrad SET Status = '" & status & "' WHERE Id = '" & Id & "'"
                ExecuteQuery(query)
                MsgBox("School successfully Removed!", vbInformation, "Deleted")
                loadDataDisplay()
                clear()
                lockEditDel()
                dbDisconnect()
    ...
    Sub save()
        ...
        Else
            If edit = True Then
                ...
                Try
                    dbConnect()
                    Dim query As String = "UPDATE schoolgrad SET name = '" & name & "' WHERE Id = '" & id & "' "
                    ExecuteQuery(query)
                    MsgBox("School successfully Updated!", vbInformation, "Updated")
                    dbDisconnect()
                Catch ex As Exception
                    clear()
                    lockEditDel()
            ...
            Else
                Try
                    dbConnect()
                    Dim query As String = "INSERT INTO schoolgrad SET Id  = '" & id & "', name ='" & name & "', Status ='ACTIVE', date_created ='" & dcreated & "'"

                    ExecuteQuery(query)
                    MsgBox("School successfully Registered!", vbInformation, "Saved")
