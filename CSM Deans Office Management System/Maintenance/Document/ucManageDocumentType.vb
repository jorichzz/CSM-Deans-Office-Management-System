Public Class ucManageDocumentType
    ...
    Sub loadDocTypeID()
        Try
            dbConnect()
            Dim query As String = "SELECT Id from documenttype ORDER BY Id DESC LIMIT 1"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
                    txtDocTypeID.Text = "DTYPE" & Val(Mid(reader.GetString(0), 6, 10)) + 1
                End While
            Else
                txtDocTypeID.Text = "DTYPE1001"
            End If
            dbDisconnect()
    ...
    Sub loadDataDisplay()
        ...
        Try
            Dim i As Integer = 0
            dbConnect()
            Dim query As String = "SELECT Id, name FROM documenttype WHERE Status = '" & status & "'"
            ExecuteQuery(query)
    ...
    Sub transferName()
        Try
            dbConnect()
            Dim query As String = "SELECT * FROM documenttype WHERE Id = '" & Id & "'"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
    ...
    Sub delete()
        If MsgBox("Are you sure you want to delete?", vbYesNo, "Dean's Record Tracking System") = vbYes Then
            Try
                Dim status = "DELETED"
                dbConnect()
                Dim query As String = "UPDATE documenttype SET Status = '" & status & "' WHERE Id = '" & Id & "'"
                ExecuteQuery(query)
                MsgBox("Document Type successfully Removed!", vbInformation, "Deleted")
                loadDataDisplay()
                clear()
                lockEditDel()
                dbDisconnect()
    ...
    Sub save()
        ...
        Else
            If edit = True Then
                ...
                Try
                    dbConnect()
                    Dim query As String = "UPDATE documenttype SET name = '" & name & "' WHERE Id = '" & id & "' "
                    ExecuteQuery(query)
                    MsgBox("Document Type successfully Updated!", vbInformation, "Updated")
                    dbDisconnect()
                Catch ex As Exception
                    clear()
                    lockEditDel()
