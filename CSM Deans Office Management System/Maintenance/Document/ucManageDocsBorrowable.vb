Public Class ucManageDocsBorrowable
    Dim edit As Boolean = False
    Dim Id As String
    Private Sub btnBack_Click(sender As Object, e As EventArgs) Handles btnBack.Click
        If bdoccat = True Then
            Dim maintenance As New ucBorrowDocument
    ...
    Sub loadDocTypeID()
        Try
            dbConnect()
            Dim query As String = "SELECT Id from bdocumentcategory ORDER BY Id DESC LIMIT 1"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
                    txtDocTypeID.Text = "BDCATG" & Val(Mid(reader.GetString(0), 7, 11)) + 1
                End While
            Else
                txtDocTypeID.Text = "BDCATG1001"
            End If
            dbDisconnect()
    ...
    Private Sub ucManageDocsBorrowableCategory_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        lockEditDel()
        gbLock()
        loadDataDisplay()
        If admin = False Then
    ...
    Sub loadDataDisplay()
        ...
        Try
            Dim i As Integer = 0
            dbConnect()
            Dim query As String = "SELECT Id, name FROM bdocumentcategory WHERE Status = '" & status & "'"
            ExecuteQuery(query)
    ...
    Sub transferName()
        Try
            dbConnect()
            Dim query As String = "SELECT * FROM bdocumentcategory WHERE Id = '" & Id & "'"
            ExecuteQuery(query)
            If reader.HasRows Then
                While reader.Read
    ...
    Sub delete()
        If MsgBox("Are you sure you want to delete?", vbYesNo, "Dean's Record Tracking System") = vbYes Then
            Try
                Dim status = "DELETED"
                dbConnect()
                Dim query As String = "UPDATE bdocumentcategory SET Status = '" & status & "' WHERE Id = '" & Id & "'"
                ExecuteQuery(query)
    ...
    Sub save()
        ...
        Else
            If edit = True Then
                ...
                Try
                    dbConnect()
                    Dim query As String = "UPDATE bdocumentcategory SET name = '" & name & "' WHERE Id = '" & id & "' "
                    ExecuteQuery(query)
            ...
            Else
                Try
                    dbConnect()
                    Dim query As String = "INSERT INTO bdocumentcategory SET Id  = '" & id & "', name ='" & name & "', Status ='ACTIVE', date_created ='" & dcreated & "'"
